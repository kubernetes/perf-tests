...
func (a *apiAvailabilityMeasurement) updateClusterAvailabilityMetrics(c clientset.Interface) {
	result := c.CoreV1().RESTClient().Get().AbsPath("/readyz").Do()
	status := 0
...
func (a *apiAvailabilityMeasurement) start(config *measurement.MeasurementConfig) error {
	if a.isRunning {
...
func (a *apiAvailabilityMeasurement) initFields(config *measurement.MeasurementConfig) error {
	a.isRunning = true
