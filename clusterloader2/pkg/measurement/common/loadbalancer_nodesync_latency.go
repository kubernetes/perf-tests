...
const (
	...
	// labelNodeExcludeBalancers is the node label to exclude a node from being a LB backend
...
type LoadBalancerNodeSyncMeasurement struct {
	...
	// excludedNodeName is the node name which used to trigger LB nodesync
...
func (s *LoadBalancerNodeSyncMeasurement) measureNodeSyncLatency() error {
	...
	// lbSvcMap contains key->service mapping for load balancer type services
	lbSvcMap := map[string]v1.Service{}
	for _, svc := range svcList.Items {
		if svc.Spec.Type == v1.ServiceTypeLoadBalancer {
			lbSvcMap[keyFunc(svc.Namespace, svc.Name)] = svc
		}
	}
	totalLbSvc := len(lbSvcMap)

	// Use event informer to keep track of nodeSync events.
	stopCh := make(chan struct{})
	defer close(stopCh)
	eventInformer := informerv1.NewEventInformer(s.client, s.selector.Namespace, 0, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc})
	...
	defer func() {
		if err = s.labelNodeForLBs(false); err != nil {
			klog.Errorf("Failed to label node %v: %w", s.excludedNodeName, err)
		}
...
func (s *LoadBalancerNodeSyncMeasurement) labelNodeForLBs(exclude bool) error {
	...
	if exclude {
		newNode.Labels[labelNodeExcludeBalancers] = "true"
	} else {
		delete(newNode.Labels, labelNodeExcludeBalancers)
	}
