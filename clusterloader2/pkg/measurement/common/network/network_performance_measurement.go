...
func (npm *networkPerfMetricsMeasurement) waitForWorkerPodsReady() error {
	...
	//var weightedPodReadyTimeout = podNum * 3
	//var checkWorkerPodReadyTimeout = time.Duration(weightedPodReadyTimeout) * time.Second
	workerPodReadyInterval, weightedPodReadyTimeout := getWeightedTimerValuesForPoll(podNum)
...
func getWeightedTimerValuesForPoll(numOfPods int) (time.Duration, time.Duration) {
	var weightedPodReadyTimeout = numOfPods * 3
