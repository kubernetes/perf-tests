...
func (n *scaleNodesMeasurement) Execute(config *measurement.Config) ([]measurement.Summary, error) {
	...
	scaleRate, err := util.GetInt(config.Params, "scaleRate")
	if err != nil {
		return nil, err
	}
	intervalMinutes, err := util.GetInt(config.Params, "intervalMinutes")
	if err != nil {
		return nil, err
	}
	...
	klog.Infof("Starting node scaling: target=%d, rate=%d/interval, interval=%dm, timeout=%v",
		targetNodeCount, scaleRate, intervalMinutes, timeout)

	// Start the scaling operation in a goroutine
	errCh := make(chan error)
	go func() {
		errCh <- scaler.ScaleNodes(scaleRate, intervalMinutes, targetNodeCount)
	}()
