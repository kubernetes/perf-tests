...
func (ste *simpleExecutor) ExecuteTestSteps(ctx Context, conf *api.Config) *errors.ErrorList {
	...
	steps, err := ste.processSteps(ctx, conf.Steps)
	if err != nil {
		// TODO(mmat): Wrap error
...
// TODO: Document
// computes the slice of actual steps to execute, flattening the modules
// into a list of executeable steps.
func (ste *simpleExecutor) processSteps(ctx Context, unprocessedSteps []api.Step) ([]*api.Step, error) {
	var processedSteps []*api.Step
	for i := range unprocessedSteps {
		step := &unprocessedSteps[i]
		if step.IsModule() {
			module, err := ste.processModule(ctx, &step.Module)
			if err != nil {
				return nil, err
			}
			processedModuleSteps, err := ste.processSteps(ctx, module.Steps)
			if err != nil {
				return nil, err
			}
...
// TODO: Document
func (ste *simpleExecutor) processModule(ctx Context, module *api.Module) (*api.Module, error) {
	if module.Path == "" {
		return module, nil
	}
...
func (ste *simpleExecutor) ExecuteStep(ctx Context, step *api.Step) *errors.ErrorList {
	...
	if step.IsMeasurement() {
		...
		return errors.NewErrorList(fmt.Errorf("TODO(mmat): Empty step"))
	}
	wg.Wait()
