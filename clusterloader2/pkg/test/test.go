...
func RunTest(f *framework.Framework, conf *api.Config) error {
	...
	err := f.CreateAutomanagedNamespaces(int(conf.AutomanagedNamespaces))
	defer f.DeleteAutomanagedNamespaces()
	if err != nil {
		return fmt.Errorf("automanaged namespaces creation failed: %v", err)
	}
	return nil
}
