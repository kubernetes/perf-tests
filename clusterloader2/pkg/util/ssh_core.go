...
// Note: This whole file was copied from:
//   https://github.com/kubernetes/kubernetes/blob/0a124e28b6995ccb7c1b3521ca0a0abd47f9b916/test/e2e/framework/ssh/ssh.go
...
func NodeSSHHosts(ctx context.Context, c clientset.Interface) ([]string, error) {
	...
	if len(hosts) < len(nodelist.Items) {
		klog.Infof("No external IP address on nodes, falling back to internal IPs")
	...
	for _, host := range hosts {
		go func(host string) {
			defer wg.Done()
			if canConnect(host) {
				klog.Infof("Assuming SSH on host %s", host)
				sshHostsLock.Lock()
				sshHosts = append(sshHosts, net.JoinHostPort(host, SSHPort))
				sshHostsLock.Unlock()
			} else {
				klog.Infof("Skipping host %s because it does not run anything on port %s", host, SSHPort)
			}
		}(host)
	}
	wg.Wait()
...
func canConnect(host string) bool {
	...
	if err != nil {
		klog.Infof("cannot dial %s: %v", hostPort, err)
		return false
	}
	conn.Close()
	return true
...
func LogResult(result Result) {
	...
	klog.Infof("ssh %s: command:   %s", remote, result.Cmd)
	klog.Infof("ssh %s: stdout:    %q", remote, result.Stdout)
	klog.Infof("ssh %s: stderr:    %q", remote, result.Stderr)
	klog.Infof("ssh %s: exit code: %d", remote, result.Code)
}
...
func IssueSSHCommandWithResult(ctx context.Context, cmd, provider string, node *v1.Node) (*Result, error) {
	klog.Infof("Getting external IP address for %s", node.Name)
	host := ""
	...
	klog.Infof("SSH %q on %s(%s)", cmd, node.Name, host)
...
func expectNoErrorWithOffset(offset int, err error, explain ...interface{}) {
	if err != nil {
		klog.Infof("Unexpected error occurred: %v", err)
	}
