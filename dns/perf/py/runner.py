...
from params import ATTRIBUTE_CLUSTER_DNS, Inputs, TestParams
...
class Runner(object):
  ...
  def __init__(self, args):
    ...
    self.test_params = TestParams.load_from_file(args.params)
  ...
  def go(self):
    ...
    try:
      ...
      for test_case in test_cases:
        try:
          inputs = Inputs(self.deployment_yaml, ['/dnsperf', '-s', self.args.dns_ip])
    ...
    if active:
      while True:
        code, out, err = self._kubectl(
            ...
            'dig', '@' + self.args.dns_ip, 'kubernetes.default.svc.cluster.local.')
        if code == 0:
          break
