...
RESULTS = [
    Result('queries_sent', int, re.compile(r'\s*Queries sent:\s*(\d+)')),
    Result('queries_completed', int,
           re.compile(r'\s*Queries completed:\s*(\d+).*')),
    Result('queries_lost', int, re.compile(r'\s*Queries lost:\s*(\d+).*')),
    Result('run_time', float, re.compile(r'\s*Run time \(s\):\s*([0-9.]+)')),
    Result('qps', float, re.compile(r'\s*Queries per second:\s*([0-9.]+)')),
...
class Parser(object):
  ...
  """
    ...
    def _compute_derived(self):
        ...
        histogram = reduce(
            list.__add__, [[rtt] * count for rtt, count in self.histogram], [])
        _log.debug('Latency histogram = %s', histogram)

        for name, ptile in [('latency_50_percentile',
                             50), ('latency_95_percentile',
                                   95), ('latency_99_percentile', 99),
...
"""
    ...
    def put(self, results, ignore_if_dup=True):
        ...
        sql = ('INSERT INTO runs (run_id, run_subid, ' +
               ','.join([p.name for p in PARAMETERS]) + ') VALUES (' +
        ...
        sql = ('INSERT INTO results (run_id, run_subid, ' +
               ','.join([r.name for r in RESULTS]) + ') VALUES (' +
               ','.join(['?'] * (2 + len(RESULTS))) + ')')
        _log.debug('results sql -- %s', sql)
        self.c.execute(sql, key + [
            results['data'][r.name] if r.name in results['data'] else None
            for r in RESULTS
        ])
    ...
    def _exists(self, key):
        self.c.execute(
            'SELECT COUNT(*) FROM runs WHERE run_id = ? and run_subid = ?',
            key)
