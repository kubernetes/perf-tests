...
class KubednsCPU(DeploymentContainerSpecParam):
    def __init__(self):
        super(KubednsCPU, self).__init__('kubedns_cpu', int, 'kubedns',
                                         ['resources', 'limits', 'cpu'])

    def set(self, inputs, value):
        super(KubednsCPU, self).set(inputs, None
                                    if value is None else '%dm' % value)


class DnsmasqCPU(DeploymentContainerSpecParam):
    def __init__(self):
        super(DnsmasqCPU, self).__init__('dnsmasq_cpu', int, 'dnsmasq',
                                         ['resources', 'limits', 'cpu'])

    def set(self, inputs, value):
        super(DnsmasqCPU, self).set(inputs, None
                                    if value is None else '%dm' % value)
...
class QueryFile(DnsperfCmdlineParam):
    ...
    def set(self, inputs, value):
        super(QueryFile, self).set(inputs, None
                                   if value is None else '/queries/' + value)


class RunLengthSeconds(DnsperfCmdlineParam):
    def __init__(self):
        super(RunLengthSeconds, self).__init__('run_length_seconds', str, '-l',
                                               False)
