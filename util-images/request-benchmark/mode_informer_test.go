...
func TestIsInformerContextDoneErr(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := isInformerContextDoneErr(tt.err); got != tt.expected {
				t.Errorf("isInformerContextDoneErr(%v) = %v, want %v", tt.err, got, tt.expected)
			}
		})
	}
}
